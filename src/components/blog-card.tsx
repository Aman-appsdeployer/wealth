...
const BlogCard: React.FC<BlogCardProps> = ({ blog }) => {
    return (
        <div className="rounded-xl ring-1 ring-zinc-900/5 bg-slate-50 group hover:scale-105 transition-all duration-300 ease-in-out transform relative p-4">
            <div className="my-auto">
                <img
                    src={`/etho/donald-trump.jpg`}
                    className="w-full h-60 rounded-l object-cover transition-opacity duration-300 ease-in-out opacity-90 group-hover:opacity-100"
                />
                <div className="absolute top-0 rounded-full right-0 px-3 py-1 m-5 backdrop-blur-xl">
                    <p className="font-medium text-white text-sm text-left">
                        {blog.id} Edition
                    </p>
                </div>
                ...
                <div className="flex flex-row items-center justify-between mt-3 pr-5">
                    ...
                    <button
                        onClick={() => (window.location.href = blog.path)}
                        className="flex justify-end items-center hover:cursor-pointer rounded-xl transition-colors duration-300 ease-in-out group-hover:text-indigo-600"
                    >
                        <p className="text-xs font-semibold underline">Read more</p>
                    </button>
                </div>
